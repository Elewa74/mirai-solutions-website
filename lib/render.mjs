...
const ASSET_VERSION = process.env.NODE_ENV === "production" ? "2026-09-13a" : String(Date.now());
...
/**
 * Inner-page header: a dark editorial band (the page's single dark band) with a page index, the kicker,
 * the title (word-by-word entrance), the intro, an oversized outlined "ghost" page name and an
 * "On this page" row of anchor pills that turns the header into a real table of contents.
 */
function pageHero(locale, { no, kicker, short, title, intro, anchors = [] }) {
  const c = siteContent[locale];
  const words = title.split(/\s+/).map((w, i) => `<span class="w" style="--i:${i}">${e(w)}</span>`).join(" ");
  const anchorRow = anchors.length
    ? `<nav class="ih-anchors" aria-label="${e(c.global.onThisPage)}"><span class="ih-anchors-label">${e(c.global.onThisPage)}</span>${anchors.map((a) => `<a href="#${e(a.id)}">${e(a.label)}</a>`).join("")}</nav>`
    : "";
  return `<section class="inner-hero" data-inner-hero data-dark>
    <canvas class="dots" data-dots aria-hidden="true"></canvas>
    <div class="ih-glow" aria-hidden="true"></div>
    <span class="ih-ghost" data-ghost aria-hidden="true">${e(short)}</span>
    <div class="shell ih-inner">
      <p class="ih-meta"><span class="ih-no gradient-text">${e(no)}</span><span class="ih-rule" aria-hidden="true"></span><span class="ih-kicker">${e(kicker)}</span></p>
      <div class="ih-grid">
        <h1 class="ih-title${title.length > 70 ? " ih-title--long" : ""}" aria-label="${e(title)}">${words}</h1>
        <p class="ih-lead">${e(intro)}</p>
      </div>
      ${anchorRow}
    </div>
  </section>`;
...
function renderSolutions(locale) {
  ...
  const L = p.labels;
  return `<main>${pageHero(locale, { no: "01", kicker: p.kicker, short: p.short, title: p.title, intro: p.intro, anchors: c.solutions.map((sol) => ({ id: sol.key, label: sol.name })) })}
...
function renderAbout(locale) {
  const c = siteContent[locale];
  const a = c.about;
  return `<main>${pageHero(locale, { no: "03", kicker: a.kicker, short: a.short, title: a.title, intro: a.intro, anchors: [{ id: "story", label: a.storyLabel }, { id: "today", label: a.todayTitle }, { id: "principles", label: a.principlesTitle }] })}
    <section class="section-pad about-story-section" id="story"><div class="shell about-story has-photo">
    ...
    <section class="section-pad soft-section" id="today"><div class="shell today-grid">
    ...
    <section class="section-pad principles-section" id="principles"><div class="shell"><p class="kicker">${e(a.principlesTitle)}</p><div class="principle-grid five">${a.principles.map((p, i) => `<article><span class="why-no">0${i + 1}</span><h2>${e(p.title)}</h2><p>${e(p.body)}</p></article>`).join("")}</div></div></section>
...
function renderNotFound(locale) {
  const c = siteContent[locale];
  return `<main><section class="inner-hero inner-hero--plain section-pad"><div class="shell"><p class="kicker">404</p><h1>${e(c.notFound.title)}</h1><p class="inner-lead">${e(c.notFound.body)}</p><a class="button" href="${href("/", locale)}">${e(c.notFound.cta)}</a></div></section></main>`;
}

function renderRedirect(locale, to) {
  const c = siteContent[locale];
  return `<main><section class="inner-hero inner-hero--plain section-pad"><div class="shell"><p class="kicker">301</p><h1>${e(c.redirect.title)}</h1><p class="inner-lead">${e(c.redirect.body)}</p><a class="button" href="${e(to)}">${e(c.redirect.cta)} <span aria-hidden="true">${arrow(locale)}</span></a></div></section></main>`;
}
...
function consultationModal(locale) {
  ...
  return `<div class="consult" data-consult-modal hidden>
    ...
    <div class="consult-dialog" role="dialog" aria-modal="true" aria-labelledby="consult-title" aria-describedby="consult-intro" tabindex="-1" data-consult-dialog>
      ...
      <form class="consult-form" data-consult-form method="post" action="/api/consultation" novalidate data-copy="${e(JSON.stringify({ errors: k.errors, states: k.states, whatsappMessage: k.whatsappMessage, sending: k.sending, autoReply: k.autoReply, copied: k.copied }))}">
        ...
        <div class="hp-field" aria-hidden="true"><label for="c-honey">Leave this field empty</label><input id="c-honey" name="_honey" type="text" tabindex="-1" autocomplete="off"></div>
...
export function renderPage(path, locale, opts = {}) {
  ...
  // Cookie-free visitor analytics (Cloudflare Web Analytics beacon); only on public pages, only when a token is configured
  const cfToken = /^[0-9a-f]{32}$/i.test(process.env.MIRAI_CF_BEACON_TOKEN || "") ? process.env.MIRAI_CF_BEACON_TOKEN : "";
  const analytics = cfToken && !special ? `\n  <script defer src="https://static.cloudflareinsights.com/beacon.min.js" data-cf-beacon='{"token": "${cfToken}"}'></script>` : "";
  return `<!doctype html>
...
<head>
  ...
  <meta property="og:image" content="${e(siteUrl)}/images/og-${locale}.jpg?v=${ASSET_VERSION}">
  <meta property="og:image:width" content="1200">
  <meta property="og:image:height" content="630">
  <meta property="og:image:alt" content="${locale === "ar" ? "Mirai Solutions — حلول رقمية عملية تُبنى حول احتياجات عملك الحقيقية" : "Mirai Solutions — practical digital solutions built around real business needs"}">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:image" content="${e(siteUrl)}/images/og-${locale}.jpg?v=${ASSET_VERSION}">
  ...
  ${(locale === "ar" ? ["noto-kufi-arabic-700-arabic", "ibm-plex-sans-arabic-400-arabic"] : ["manrope-800-latin", "inter-400-latin"]).map((f) => `<link rel="preload" as="font" type="font/woff2" crossorigin href="/fonts/${f}.woff2">`).join("\n  ")}
...
<body data-locale="${lang}" data-page="${e(logical)}" data-theme-mode="${THEME_MODE}"${whatsapp ? ` data-whatsapp="${whatsapp}"` : ""}${formEndpoint ? ` data-form-endpoint="${e(formEndpoint)}"` : ""}${formEndpoint && formCc ? ` data-form-cc="${e(formCc)}"` : ""}${notifyUrl ? ` data-notify-url="${e(notifyUrl)}"` : ""}>
  ...
  <script type="module" src="/site.js?v=${ASSET_VERSION}"></script>${analytics}
</body>
</html>`;
}
